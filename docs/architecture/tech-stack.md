...
Выбор зафиксирован. Замена технологии — только через ADR. **Обновление версии внутри принятой технологии — НЕ ADR** (решение владельца, issue #8): применяется с обоснованием, записанным в этом файле, и зелёным CI. Критерии выбора: зрелость, качество поддержки ИИ-агентами, скорость разработки малой командой, open-source лицензии.
...
| Компонент | Выбор |
|---|---|
| Пакетный менеджер / оркестрация | pnpm 11 workspaces + Turborepo |
| Рантайм | Node.js 24 LTS (≥ 24.15 — требование schematics NestJS 12 для `nest generate`/`upgrade`; пин мажорки: `.node-version`) |
| Язык | TypeScript strict ~5.9 (TS 7 отложен: typescript-eslint peer `< 6.1`; пересмотр — при релизе typescript-eslint с поддержкой TS 7) |
