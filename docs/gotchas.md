...
- **Инструмент `edit` — только после инструмента `read`**: вывод `Get-Content` в консоли Windows показывает UTF-8 файлы мозиброй cp866 (кириллица и эмодзи искажены); скопированный оттуда `oldString` не матчится с точными байтами файла — edit падает «not found». Механизм подтверждён воспроизведением (сессия issue #2, 2026-09-02; ловушка собственного инструментария — внешний источник неприменим). Тяжёлые правки кириллических файлов — `write` всего файла.
