...
- **SMIL `<animateMotion>` не стреляет DOM `animationend`** — для «один пробег и убрать» таймаут/JS, не событие; перемонтирование через `key` перезапускает SMIL-анимацию надёжнее, чем смена атрибутов.
