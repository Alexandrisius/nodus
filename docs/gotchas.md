...
- **Vite dev-сервер на Windows может застрять на старой трансформации модуля после перезаписи файла** (watcher пропускает событие изменения): страница рендерит старый JSX без ошибок, полный reload не помогает, лечится перезапуском dev-сервера. Симптом для диагностики: код в файле новый, а в DOM его нет (проверять `locator(...).textContent()`). Воспроизведено дважды в issue #4 (перезаписи `logo-icon.tsx`, `home-page.tsx`; подтверждено воспроизведением).
