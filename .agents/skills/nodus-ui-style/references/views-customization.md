...
Инфраструктура всех модулей: `apps/web/src/shared/views/` (`view-store.ts`, `use-view-fields.ts`, `view-settings.tsx`, `column-resizer.tsx`, `data-table.tsx`, `data-table-header.tsx`, `use-column-draft.ts`, `sort-rows.ts`). Эталон применения — `features/tasks` (дерево-реестр `lib/task-fields.tsx`, список, канбан).
