...
### Чип стадии и шапка колонки канбана (`features/tasks/lib/stage-tone.ts`)
...
### Поля карточки-слайдера (`features/tasks/components/task-fields.tsx`)

Инспектор полей: чистые строки «иконка (lucide 3.5, muted) + моно-метка w-44 / значение» без табличных рамок, ховер-заливка строки. Сетка на **container queries** левой зоны карточки (`@container` на зоне): узко — 1 колонка, `@min-[640px]` — 2; зазор фикс (`gap-x-12`), блок `max-w-3xl mx-auto` — поля тянутся до предела, дальше растут боковые отступы и блок центрируется (модель ClickUp). Ресайзимые зоны перестраиваются только container queries, НЕ media вьюпорта.

### Панель-навигатор ветки (`features/tasks/components/task-branch-drawer.tsx`)

Левая ВТАЛКИВАЮЩАЯ колонка слайдера (не оверлей — контент и разделитель чата сдвигаются плавно: grid-трек 0↔300px, `transition-[grid-template-columns]`, треков постоянно 4 — иначе не анимируется; панель монтируется раз и остаётся): дерево от корневого предка любой вложенности (эндпоинт `/tasks/:id/branch`), строка = chevron + точка стадии в тоне + название + номер, текущая подсвечена; клик — переход на ту задачу в этом же слайдере (панель не закрывается — она навигатор); «+» на строке — инлайн-создание дочерней; внизу — быстрое создание к текущей. Тоггл — иконка `ListTree` в полосе цепочки слева.
