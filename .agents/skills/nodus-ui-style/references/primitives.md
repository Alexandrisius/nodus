...
Левая выдвижная панель слайдера (зеркало «О задаче» справа): дерево от корневого предка любой вложенности (эндпоинт `/tasks/:id/branch`), строка = chevron + точка стадии в тоне + название + номер, текущая подсвечена; клик — переход на ту задачу в этом же слайдере (панель не закрывается — она навигатор); «+» на строке — инлайн-создание дочерней; внизу — быстрое создание к текущей.
