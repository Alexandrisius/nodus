...
Левая ВТАЛКИВАЮЩАЯ колонка слайдера (не оверлей — контент и разделитель чата сдвигаются плавно: auto-трек сетки + `transition-[width]` 0↔300px на самой колонке — НЕ transition треков сетки: трек чата меняется drag'ом на каждый pointermove, анимация трека = фризы, gotchas; панель монтируется раз и остаётся): дерево от корневого предка любой вложенности (эндпоинт `/tasks/:id/branch`), строка = chevron + точка стадии в тоне + название + номер, текущая подсвечена; клик — переход на ту задачу в этом же слайдере (панель не закрывается — она навигатор); «+» на строке — инлайн-создание дочерней; внизу — быстрое создание к текущей. Тоггл — иконка `Waypoints` (дуга-граф) в полосе цепочки слева.
