...
- **`isRSC`** → when `true`, components using `useState`, `useEffect`, event handlers, or browser APIs need `"use client"` at the top of the file. Always reference this field when advising on the directive. **Nodus:** our `apps/web` is a Vite SPA — `isRSC` is always `false` here, so the `"use client"` directive is never needed; ignore this rule's RSC branch.
