...
This matters when `getFlag` hits the network, a feature-flag service, or DB work: skipping it when `someCondition` is false removes that cost on the cold path.
