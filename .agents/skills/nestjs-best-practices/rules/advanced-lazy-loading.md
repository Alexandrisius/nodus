...
> **Nodus note:** limited applicability here. Feature modules are toggled by feature flags (I10), and event handlers are wired by core at bootstrap — a lazily loaded module would miss handler registration. Apply lazy loading only to heavy, self-contained infrastructure (e.g., a report generator module with no event subscriptions), never to modules with `events/*.handler.ts` files.
