 ...
 * ведущая группа сверху в МЯГКОЙ тональной области (без заголовка-секции и
 * без жёсткой рамки).
 */
...
export function ConversationList({
  ...
  // Мягкая область закреплённых ЧАТОВ (#96, вердикт владельца 24.09.2026, реф
  // Битрикс24): закреплённые идут ВЕДУЩЕЙ группой (sortByActivity: pinned —
  // первыми), и именно эта группа оборачивается тональной областью. Механика и
  // сортировка закрепов не меняются — группировка ЧИСТО визуальная; нет
  // закреплённых — нет и области. Это НЕ пин-бар сообщений (pin-bar.tsx):
