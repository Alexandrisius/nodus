...
«Рисованность» стиля — без SVG-фильтров: утилита `paper-card` даёт бумажную карточку с двумя карандашными обводками псевдоэлементами и неровными радиусами углов, `crayon-fill` — зернистую восковую заливку, `sketch-tilt` — микроповороты карточек в списках, `texture-paper` — зерно бумаги. Иллюстрации вживляются в бумагу через `mix-blend-multiply` без рамок.
