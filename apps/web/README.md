...
Всё визуальное — дизайн-токенами `@nodus/ui` (тема как данные): `:root` — «корпоративная», `[data-theme='airy']` — «воздушная» (переключатель в топбаре, проверка подмены токенов), `.dark` заготовлена. Компоненты не содержат значений токенов.
