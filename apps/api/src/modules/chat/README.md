...
- **Просмотранность — watermark (#102 раунд 2: просмотр = ВИДИМОСТЬ в
  вьюпорте)**: `conversation_members.last_read_seq` (GREATEST, не
  откатывается) + `last_read_at`. Unread = COUNT(seq > last_read_seq OR
  ...
  счётчики дают дрейф). Курсор двигают ТОЛЬКО квитанции `POST /read` —
  выдача ленты (`GET messages`) его НЕ трогает (открытый чат не
  «прочитывает» спрятанное за прокруткой); квитанция клампится к
  `conversations.last_seq`, повтор/отставшая квитанция — тихие (GREATEST).
  **«Увидел где угодно = просмотрено»**: квитанция из треда двигает watermark
  БЕСЕДЫ (seq общий). **readAt/readBy**: readAt — момент ПЕРВОГО
  просмотревшего (min по просмотревшим; в direct он единственный);
  `readBy: UserRef[]` — просмотревшие СВОИХ сообщений на момент выдачи (у
  чужих — пустой, просмотревших видит только автор); правка (editedAt)
  исключает просмотревшего до пересмотра. Считается выводно из watermark-ов
  участников, которых лента уже загружает на страницу — новых запросов и
  таблиц нет; WS-обновление — событием `chat.message_read` (readAt в payload
  — фактическое время строки, не момент эмита). `seq` сообщения присутствует
  в DTO (`messageSchema.seq`) — источник upToSeq квитанций клиента.
