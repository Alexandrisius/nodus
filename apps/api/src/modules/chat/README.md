...
- **Прочитанность — watermark**: `conversation_members.last_read_seq` (GREATEST,
  ...
  (`GET messages`). **readAt/readBy (#102, модель Битрикс24/Telegram)**:
  readAt — момент ПЕРВОГО прочитавшего (min по прочитавшим; в direct он
  единственный); `readBy: UserRef[]` — прочитавшие СВОИХ сообщений на момент
  выдачи (у чужих — пустой, прочитавших видит только автор); правка
  (editedAt) исключает прочитавшего до перечитывания. Считается выводно из
  watermark-ов участников, которых лента уже загружает на страницу — новых
  запросов и таблиц нет; WS-обновление — событием `chat.message_read`.
