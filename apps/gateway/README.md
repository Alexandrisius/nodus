...
| клиент → сервер | `chat.typing`                         | `{conversationId}` — троттл ~3 с на пользователя-беседу; рассылка: conv-комната + user-комнаты участников без автора                     |
...
**`chat.typing` (раунд 2 #104, модель Telegram)**: помимо комнаты беседы —
user-комнаты остальных участников (SELECT members, механизм message_sent) —
индикатор живёт и в СПИСКЕ чатов; автор из user-рассылки исключён (свои
вкладки свою печать не видят), своя печать не показывается и на клиенте.
